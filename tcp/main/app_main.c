...
static void continuous_adc_init(adc_channel_t *channel, uint8_t channel_num, adc_continuous_handle_t *out_handle)
{
    ...
    adc_continuous_config_t dig_cfg = {
        .sample_freq_hz = 611, // ver en sdkconfig valores máximos y mínimos (611Hz-83333Hz)
...
static void read_adc_task(void)
{
	...
	//**************************************************************************************
	while(1){
	ESP_LOGI(TAG, "estamos en adc read");
	...
	printf("ret %d\n", ret);
	   if ( ret==0 ){
		//Tiempo de registro
		if ( strncmp(revice_msg.arrayDatos[1], "LR", 2)){
		count=800;
		}
		if ( strncmp(revice_msg.arrayDatos[1], "SR", 2)){
		count=5;
		}
...
static void servo_ind_sensor_task(void)
{
	...
	printf("ret %d\n", ret);
		if ( ret==0 ){
			//sentido de giro
			if ( strncmp(revice_msg.arrayDatos[1], "B", 1)){
			gpio_set_level(GPIO_OUTPUT_IO_1,1);
			}
			if ( strncmp(revice_msg.arrayDatos[1], "F", 1)){
			gpio_set_level(GPIO_OUTPUT_IO_1,0);
			}
			if ( strncmp(revice_msg.arrayDatos[2], "LP", 2)){
			pasos_motor=500;
			}
			if ( strncmp(revice_msg.arrayDatos[2], "SP", 2)){
			pasos_motor=10000;
			}
...
static void servo_wheel_task(void)
{
   ...
   int32_t velocidad_motor=0;  
	...
	printf("ret %d\n", ret);
		if ( ret==0 ){
			//sentido de giro
			if ( strncmp(revice_msg.arrayDatos[1], "I", 1)){
			gpio_set_level(GPIO_OUTPUT_IO_1,1);
			}
			if ( strncmp(revice_msg.arrayDatos[1], "D", 1)){
			gpio_set_level(GPIO_OUTPUT_IO_1,0);
			}
			if ( strncmp(revice_msg.arrayDatos[2], "SV", 2)){
			velocidad_motor=8;
			}
			if ( strncmp(revice_msg.arrayDatos[2], "HV", 2)){
			velocidad_motor=10;
			}
				for(int i=0;i<5000;i++){// tres vueltas y un poquito mas
				printf("cnt: %d\n", cnt++);
